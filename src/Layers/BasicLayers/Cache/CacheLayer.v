...
Section CacheLayer.
  ...
  Inductive exec' :
    forall T, oracle' ->  state' -> cache_prog T -> @Result state' T -> Prop :=
  | ExecRead : 
      forall d a,
  ...
  Definition weakest_precondition' T (p: cache_prog T) :=
   match p in cache_prog T' return (T' -> state' -> Prop) -> oracle' -> state' -> Prop with
   | Read a =>
    fun Q o s =>
        o = [Cont] /\
        Q (s a) s
  ...
  Definition strongest_postcondition' T (p: cache_prog T) :=
   match p in cache_prog T' return (oracle' -> state' -> Prop) -> T' -> state' -> Prop with
   | Read a =>
     fun P t s' =>
       exists s,
        P [Cont] s /\
