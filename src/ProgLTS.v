...
Definition state_equivalence_simulation : 
  SelfSimulation prog_LTS state_equivalent.
  intros; constructor; intros.
  simpl in *; eapply exec_lts_preserves_equivalence; eauto.
Defined.

Definition equivalence_noninterference {lts T} (p: Op lts T) (eq: State lts -> State lts -> Prop):=
  forall st1 st1' st2,
    (transition _) _ st1 p st1' ->
    eq st1 st2 ->
    exists st2',
      (transition _) _ st2 p st2' /\
      eq (extract_state st1') (extract_state st2') /\
      (forall def, extract_ret def st1' = extract_ret def st2').

Theorem equivalence_to_noninterference:
  forall lts eq,
    Simulation lts lts eq ->
    (forall T (p: Op lts T), equivalence_noninterference p eq).
Proof.
  unfold equivalence_noninterference; intros.
  inversion H; clear H.
  specialize simulation_correct with (1:= H0)(2:=H1).
  cleanup.
