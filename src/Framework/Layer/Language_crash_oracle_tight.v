...
Lemma exec_deterministic_wrt_oracle_prefix:
  ...
  -
    ...
    specialize IHp with (1:= H0)(2:= H1)(3:=H2); cleanup; eauto.
...
Proof.
  ...
  +
    ...
    specialize IHp with (1:=H0)(3:=H2).
    apply IHp; eauto.
    intros; apply H1.
    apply app_eq_nil in H4; cleanup; eauto.
  +
    rewrite <- app_assoc in H5; eauto.
    eapply exec_finished_deterministic in H0; eauto; cleanup; eauto.
    apply app_inv_head in H5; cleanup; eauto.
...
Proof.
  ...
  -
    split_ors; cleanup.
    exfalso; clear H2; eapply finished_not_crashed_oracle_app; eauto.
...
Qed.
  ...
  Proof.
    ...
    econstructor; eexists; intuition eauto.
    erewrite H8, H5; simpl; eauto.
    eapply ExecBindCrash; eauto.
    erewrite H8, app_nil_r; eauto.

    specialize H with (1:=H9)(2:= H1).
