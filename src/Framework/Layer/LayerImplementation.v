Require Import Primitives Lia.
Require Import  Layer.Core.
Import ListNotations.

Set Implicit Arguments.

Section LayerImplementation.
  ...
  | ExecRecovered :
      forall T (p: prog' T) p_rec
        ...
        exec_with_recovery' u lo (get_reboot_state d') l_grs p_rec p_rec ret ->
        exec_with_recovery' u (o::lo) d (get_reboot_state::l_grs) p p_rec (Recovered (extract_state_r ret)).
  ...
  Record Layer :=
    {
      ...
      prog := prog';
      exec := exec';
      exec_with_recovery := exec_with_recovery';
    }.
...
Arguments Ret {O T}.
Arguments Op O {T}.
Hint Extern 1 (LayerImplementation.exec _ _ _ _ (Op _ _) (Finished _ _)) => eapply (ExecOp _) : core.
...
Notation "x <-| p1 ; p2" := (Bind (Op _ p1) (fun x => p2))(right associativity, at level 60).
 ...
 Lemma exec_empty_oracle:
      forall O (L: Layer O) T (p: prog L T) u s s',
      ~ exec L u [] s p s'.
...
Lemma exec_finished_deterministic_prefix:
  forall O (L: Layer O) T (p: prog L T) u o1 o2 o3 o4 s s1 s2 r1 r2,
...
Lemma finished_not_crashed_oracle_prefix:
  forall O (L: Layer O) T (p: prog L T) u o1 o2 o3 o4 s s1 s2 r1,
    exec L u o1 s p (Finished s1 r1) ->
    o1 ++ o3 = o2 ++ o4 ->
...
Lemma finished_not_crashed_oracle_app:
  forall O (L: Layer O) T (p: prog L T) u o1 o2 s s1 s2 r1,
    exec L u o1 s p (Finished s1 r1) ->
...
Lemma exec_deterministic_wrt_oracle_prefix:
      forall O (L: Layer O) T (p: prog L T) u o1 o2 x1 x2 s ret1 ret2,
...
Lemma exec_deterministic_wrt_oracle:
  forall O (L: Layer O) T (p: prog L T) u o s r1 r2,
      exec L u o s p r1 ->
      exec L u o s p r2 ->
      r1 = r2.
Proof.
...
Lemma exec_with_recovery_deterministic_wrt_reboot_state :
  forall O (L: Layer O) u lo s T (p: L.(prog) T) rec l_get_reboot_state ret1 ret2,
    exec_with_recovery L u lo s l_get_reboot_state p rec ret1 ->
    exec_with_recovery L u lo s l_get_reboot_state p rec ret2 ->
    ret1 = ret2.
Proof.
  ...
  erewrite IHexec_with_recovery'; eauto.
  Unshelve.
  all: eauto.
Qed.

Definition minimal_oracle {O} {L: Layer O} {T} (p: L.(prog) T) u s o om :=
forall s',
      exec L u o s p s' ->
      exists o',
      o = om ++ o' /\
...
forall O (L: Layer O) T (p: prog L T) o u s s' r,
...
forall O (L: Layer O) T (p: prog L T) u o s r,
exec L u o s p r ->
...
forall O (L: Layer O) T (p: prog L T) u o om1 om2 s r,
exec L u o s p r ->
...
Lemma exec_crashed_minimal_oracle:
  forall O (L: Layer O) T (p: prog L T) u o s s',
