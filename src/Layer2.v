...
Section Layer2.
  ...
  Inductive exec2 :
    forall T, state2 -> prog_limited T -> @Result state2 T -> Prop :=
  | Exec2Finished :
  ...
  Definition prog2_LTS := {| State:= state2; Op:= prog_limited; transition := exec2 |}.
  ...
  Definition state_noninterference {T} (p: prog_limited T) (valid: state2 -> Prop):=
  ...
  Definition ret_noninterference {T} (p: prog_limited T) (valid: state2 -> Prop) :=
    forall st1 st1' st2,
    valid st1 ->
    valid st2 ->
    exec2 st1 p st1' ->
  ...
  Definition data_noninterference {T} (p: prog_limited T) (valid: state2 -> Prop) :=
